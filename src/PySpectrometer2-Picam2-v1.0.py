...
while True:
    ...
    #=int((frameHeight/2)-40) #origin of the vertical crop
    #y=200  #origin of the vert crop
    #x=0    #origin of the horiz crop
    #h=80   #height of the crop
    #w=frameWidth   #width of the crop
    #cropped = frame[y:y+h, x:x+w]
    zoom_factor = 2.45  # Adjust this factor as needed (1.0 = no zoom, 2.0 = 2x zoom)

    # Compute new dimensions for zoomed ROI
    new_width = int(frameWidth / zoom_factor)
    new_height = int(80 / zoom_factor)  # Keeping same aspect ratio

    # Centering the zoomed region
    x_center = frameWidth // 2
    y_center = int(frameHeight / 2) - 40  # Adjust as per your spectrometer alignment

    x1 = max(0, x_center - new_width // 2)
    y1 = max(0, y_center - new_height // 2)

    x2 = min(frameWidth, x_center + new_width // 2)
    y2 = min(frameHeight, y_center + new_height // 2)

    # Crop and resize
    cropped = frame[y1:y2, x1:x2]
    cropped = cv2.resize(cropped, (frameWidth, 80), interpolation=cv2.INTER_CUBIC)
    ...
    label_text = "Absorbance (A)" if absorbance_mode else "Intensity"
    cv2.putText(spectrum_vertical, label_text, (10,15), font, 0.4, (0,0,0), 1, cv2.LINE_AA)
